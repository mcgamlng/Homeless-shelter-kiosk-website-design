...
const scenes = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "It scores the earliest legal openings by how well they fill a gap.",
      "Tap order and the previous calendar order are not tie-breakers.",
    ...
    narration:
      "Server slash scheduler dot J S contains the most specialized business logic. ScheduleActivities receives the selected activities, guest identifier, existing appointments, buffer setting, current time, and workday settings. It separates timed from untimed services. For every remaining timed activity, it calls findEarliestSlot. That function checks the activity lane and the same guest's appointments. When several activities can start together, getSlotFitScore prefers the activity that closes an existing calendar gap most cleanly, then the activity with the least flexible closing window. The guest's tap order is never used as an automatic tie-breaker, and the daily rebalancer does not preserve an old calendar order. This allows several service lanes to start different guests at the same earliest time while every guest still receives the required buffer. The result obeys activity-specific hours, the overall workday, check-in time, duration, and every no-overlap rule."
  },
  {
    section: "Scheduling",
  ...
  },
  {
    ...
    narration:
      "The GitHub repository is the shareable source, not the live shelter data. The MIT license allows people to use, modify, and redistribute the project while keeping the copyright and license notice. Gitignore excludes node modules, production builds, environment secrets, SQLite data, Android build folders, A P K binaries, and temporary video source files. The two finished walkthrough videos and their transcripts are deliberately public. Dot env example shows the names of settings without publishing real values. The Hmong voice pack is downloaded separately and follows Yuhalu's non-commercial terms, documented in Third Party Notices. Before every push, inspect Git status and the staged diff. Never publish a real database, guest list, administrator secret, tunnel token, Wi-Fi password, or private operational report."
  },
  {
    section: "Maintenance",
